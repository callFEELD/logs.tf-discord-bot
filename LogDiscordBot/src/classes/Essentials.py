...
def LogPlayerSearch(steamid64, limit):
    ...
    with urllib.request.urlopen("http://logs.tf/api/v1/log?player="+str(steamid64)+"&limit=" + str(limit)) as url:
        data = json.loads(url.read().decode())
    return data


# Returns a Player Demos.tf search, by inputting steamid64
def DemoPlayerSearch(steamid64):
    # going on the Logs.tf API and search for the player with a limit
    with urllib.request.urlopen("https://api.demos.tf/profiles/" + str(steamid64)) as url:
        data = json.loads(url.read().decode())
    return data


def get_closest_demo(steam_id64, logs_unix_timestamp):
    demos = DemoPlayerSearch(steam_id64)
    diffs = []
    ids = []
    for demo in demos:
...
class LogBotEssentials:
    ...
    def findMatch(self, minplayers, numoflogs, game_format, team, message):
        ...
        for player in team:
            data = LogPlayerSearch(player["steam_id"], str(numoflogs))
            logids = data["logs"]
